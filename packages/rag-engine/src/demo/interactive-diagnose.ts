...
async function main() {
  ...
  let adapter: MultiKeyLLMAdapter | ReturnType<typeof selectLLMAdapter>['adapter'];
  if (useHttp) {
    const apiKey1 = await ask('输入第一个DeepSeek API key（主key）：');
    const apiKey2 = await ask('输入第二个DeepSeek API key（备用key，直接回车跳过）：');
    const configs = [
      {
    ...
    adapter = new MultiKeyLLMAdapter(configs);
    console.log(`已切换到真实API模式（${configs.length}个key轮询兜底）\n`);
  } else {
    const selected = selectLLMAdapter(process.env);
  ...
  while (running) {
    ...
    const mileage = mileageStr.trim() ? parseInt(mileageStr, 10) : undefined;
